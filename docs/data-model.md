...
Nothing for a session is stored outside it.
...
│   └── secrets.env                0600, root            ← optional per-session user creds, env-injected
...
| Table | Key | Holds |
|---|---|---|
| `threads` | `thread_id` (PK) | One row per session: `chat_id`, `mode` (`chat`/`code`), `model`, `cwd`, the public `sid`, the resumable `code_session_id` / `chat_session_id`, `name`, `created_by`, `created_at`, and per-session toggles added by migration (`permission_mode` — default `acceptEdits`, `effort`, `max_turns`, `big_memory`, `session_notes` (agent-saved per-topic "session memory" — short notes the model persists via the in-process `remember` tool in code sessions, re-injected into the system prompt each build, size-capped, cleared by `/forget`), `favorite`, `no_sandbox`, `tools_enabled`, `add_dirs`, `fork_pending`, `auto_compact`, `hot_cache_timer`, `stream_enabled`). Indexed `(chat_id)`. |
| `usage` | `id` (PK) | One row per turn: `thread_id`, `ts`, `input_tokens`, `output_tokens`, `cache_read`, `cache_creation`, `cost_usd`, `model`, `context_tokens`. Append-only. Indexed `(thread_id, ts)` so per-thread + time-window aggregates are index range scans. Per-user totals roll up via `JOIN threads ON chat_id`; the `/sessions` and `/users` lists use single batch GROUP BY queries (`get_usage_totals_bulk` / `get_all_users_units`) rather than one aggregate per row. |
...
public render and names the on-disk session directory.

### Per-user access records (`allowlist.json`)

One record per allowlisted user:
...
- Credential token lives in `~/.claude/.credentials.json` (service user, `0600`), refreshed
  before expiry by `token_refresh.py`.
- With the credential broker on, it isn't placed in a jail: the jail receives a
